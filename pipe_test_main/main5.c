...
// 例3: サーバログからIPアドレスを抽出してカウント
int main()
{
	t_token *token1 = create_token("cat", TOKEN_WORD);
	t_token *token2 = create_token("<", TOKEN_REDIRECT_IN);
	t_token *token3 = create_token("access.log", TOKEN_WORD);

	t_token *token4 = create_token("|", TOKEN_PIPE);
	t_token *token5 = create_token("awk", TOKEN_WORD);
	t_token *token6 = create_token("{print $1}", TOKEN_WORD);
	...
	t_token *token11 = create_token("-c", TOKEN_WORD);

	t_token *token12 = create_token("|", TOKEN_PIPE);
	t_token *token13 = create_token("sort", TOKEN_WORD);
	t_token *token14 = create_token("-nr", TOKEN_WORD);

	t_token *token15 = create_token(">", TOKEN_REDIRECT_OUT);
	t_token *token16 = create_token("ip_counts.txt", TOKEN_WORD);
