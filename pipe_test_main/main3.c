...
//cat < input.txt | grep 'pattern' | awk '{print $1}' | sort | uniq > output.txt
// 例1: ファイルの内容をフィルタリングして結果を新しいファイルに保存
int main()
{
	...
	t_token *token6 = create_token ("pattern", TOKEN_WORD);

	t_token *token7 = create_token("|", TOKEN_PIPE);
	t_token *token8 = create_token("awk", TOKEN_WORD);
	t_token *token9 = create_token("{print $1}", TOKEN_WORD);

	t_token *token10 = create_token("|", TOKEN_PIPE);
	t_token *token11 = create_token("sort", TOKEN_WORD);

	t_token *token12 = create_token("|", TOKEN_PIPE);
	t_token *token13 = create_token("uniq", TOKEN_WORD);

	t_token *token14 = create_token(">", TOKEN_REDIRECT_OUT);
	t_token *token15 = create_token("output.txt", TOKEN_WORD);
